...
> **A quick note to address the question I'm sure some of you are asking: "Wait, aren't you sending data over the internet when you make LLM API calls?"** Every prompt I send to my LLM provider travels over HTTPS; the same encryption that protects your banking transactions. The model receives my question, thinks about it, and hands back an answer. That's a self-contained transaction, like making an online credit card purchase. But giving an AI agent unrestricted access to your email, your social media, your GitHub, etc.? That's more like handing your wallet, your car keys, and a signed blank check to a stranger. Unsupervised AI agents have sent threatening emails, made wildly expensive purchases, posted embarrassing content, pushed broken code to production, and in at least one well-publicized case, created real legal consequences for the person who set them up. A cloud LLM is like a consultant with anterograde amnesia who answers one question at a time, and then forgets the whole conversation, so the LLM API calls Cray makes for me are the safe part. By contrast, the agent layer, once armed with the consultant's answers, becomes an independent actor with memory and opinions, and that's where the danger comes in. The airlock system is designed to mitigate those risks while still giving my AI collaborator the freedom to work effectively.
...
I'm a MiniDisc enthusiast. I've been collecting and using MiniDisc players since 1999, when I bought my [Sony MZ-R55](https://www.minidisc.wiki/equipment/sony/portable/mz-r55) with my Circuit City employee discount (IYKYK). This format holds genuine meaning for me, beyond mere nostalgia, as a tool that was foundational to my early music recording and production workflows. The MiniDisc's track-marking feature let me record hours of improvisation, then mark the best takes and delete the rest, effectively letting me pack dozens of polished demo tracks onto a single disc without re-dubbing. That might not sound like much now, but for a musician on a budget in 1999, it was downright revolutionary.

One morning recently, I woke up to find that Cray had built PS-MiniDisc during an overnight session: a complete PowerShell module for interacting with MiniDisc hardware via USB. It included functions for reading disc contents, managing tracks, and even integrating the [netmd-exploits](https://github.com/asivery/netmd-exploits) library for activating hidden features like automatic song recognition and labeling, full-disc backup/restoration, and more.
...
**Autonomy is the multiplier.** The ability to say "here's what I'm thinking about" and then have my AI partner work on it overnight, ready with results in the morning, changes the shape of how I approach projects. The point isn't to replace my work, but rather, to delegate and parallelize it.
